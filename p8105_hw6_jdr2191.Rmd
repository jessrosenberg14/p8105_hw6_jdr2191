...
%>%
  group_by(".id") %>%
  summarize(logb0b1 = log(estimate))
```

weather_df3 =
  weather_df2 %>%
  group_by(term) %>%
  mutate(logb0 = if_else(term != "tmin", log(estimate), 0),
        logb1 = if_else(term == "tmin", log(estimate), 0))

  df %>%
  mutate(g = if_else(a == 2 | a == 5 | a == 7 | (a == 1 & b == 4), 2,
               if_else(a == 0 | a == 1 | a == 4 | a == 3 |  c == 4, 3, NA_real_)))

weather_df_intercept =
  weather_df2 %>%
  filter(term != "tmin") %>% 
  mutate(log_b0 = log(estimate)) 

weather_df_tmin = 
  weather_df2 %>%
  filter(term == "tmin") %>%
  mutate(log_b1 = log(estimate))
  
weather_df_combined = 
  merge(weather_df_intercept, weather_df_tmin)

weather_df2 %>% 
  if("term" = "(Intercept)")
  mutate(log_b0 = log(estimate) if "term" = (Intercept))))
    
    
  )
  group_by(term) %>%
  summarize(log_value = log(estimate)) %>%
  ...
  knitr::kable(digits = 3)
    ...
    ,
    results = map(models, broom::glance(r.squared))) %>% 
  select(results) %>% 
  unnest(results) 
